((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_416",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var J,A,C,E,F,B={
doK(){return new B.WJ(null)},
pA:function pA(d,e){this.a=d
this.b=e},
WJ:function WJ(d){this.a=d},
aIi:function aIi(d,e,f,g){var _=this
_.d=d
_.e=e
_.f=f
_.r=$
_.a=null
_.b=g
_.c=null},
cfp:function cfp(d){this.a=d},
cff:function cff(d,e){this.a=d
this.b=e},
cfe:function cfe(d){this.a=d},
cfo:function cfo(d,e){this.a=d
this.b=e},
cfn:function cfn(d,e,f){this.a=d
this.b=e
this.c=f},
cfm:function cfm(d){this.a=d},
cfl:function cfl(d){this.a=d},
cfj:function cfj(d){this.a=d},
cfk:function cfk(d){this.a=d},
cfi:function cfi(d){this.a=d},
cfh:function cfh(d,e){this.a=d
this.b=e},
cfg:function cfg(d){this.a=d},
dGF(d){if(d.length>1000)return A.c("toLong",!0)
return null}},D,G,H
J=c[1]
A=c[0]
C=c[2]
E=c[11]
F=c[224]
B=a.updateHolder(c[87],B)
D=c[389]
G=c[288]
H=c[247]
B.pA.prototype={
R(){return"LastDateToUpdateTermState."+this.b}}
B.WJ.prototype={
a0(){var x=A.cI(null,0,!0),w=$.I().w.p3.at,v=$.ak()
return new B.aIi(x,new A.ct(new A.d7(w,C.aL,C.ax),v),new A.M(w,v,y.o),C.o)}}
B.aIi.prototype={
gbSq(){var x=this,w=null,v=x.r
return v===$?x.r=A.dn(x.e,w,w,w,w,A.c("termExample",!0),w,B.dML(),w,w,w,1000,w,new B.cfp(x),w,w,w,!0,w,w,!1,w,!1,!1,!0,!0,w,w,C.af):v},
n(d){var x,w,v,u,t,s=this,r=null
$.cDz.b=d
x=s.d
w=y.u
v=A.a8(A.a([new A.m(43,r,r,r),A.B(A.c("term",!0),r,!1,!1,r,!1,!1,r,!1,!1,!1,1,r,!1,16,r,"")],w),C.e,C.w,C.d,r)
v=A.ev(A.a([s.Ad(d,s.f),A.hO(C.bV,A.c("termExplain",!0))],w),r,r,r,v,!0)
u=H.cn($.d3x(),r,r,r,"",r,!0,r,r,r,r,r)
t=$.eO()
return new A.mn(A.bu(r,A.dm(v,A.fQ(A.a([u,new A.m(r,30,r,r),A.K(A.a([A.ai(r,r,r,A.K(A.a([A.B(A.c("termContent",!0),r,!1,!1,A.q(d).ax.f,!1,!1,r,!1,!1,!1,1,r,!1,16,r,""),new A.m(r,10,r,r),s.gbSq(),new A.m(r,10,r,r)],w),C.e,C.h,C.d),r,r,0,!1,r,r,r,r,r,!1,r,r,new A.x(16,10,16,10),15,r,!1,!1,t)],w),C.e,C.h,C.d)],w),C.aX,r,x,!1),r,r,!1,r,r,23,!1,!0,x,!0,!0),C.t,!1,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,r,new B.cfe(d),r,r,r,r,r,r,r,r,!1,C.E),new B.cff(s,d),r)},
Ad(d,e){return new A.P(e,new B.cfo(this,e),null,null,y.C)},
a3C(d,e,f){return this.bMP(0,e,f)},
bMP(d,e,f){var x=0,w=A.i(y.e),v,u=this,t,s,r,q,p,o,n,m
var $async$a3C=A.d(function(g,h){if(g===1)return A.e(h,w)
while(true)switch(x){case 0:p=f.a.a
o=$.I()
n=o.w.p3
m=n.at
if(p===m){v=!0
x=1
break}t=m!==""&&n.ax
s=new A.M(D.tg,$.ak(),y.i)
x=3
return A.b(u.PX(e,t?s:null),$async$a3C)
case 3:r=h
if(r===!1){v=!0
x=1
break}else if(r==null){v=!1
x=1
break}q=A.kC(o.w.p3)
q.at=f.a.a
if(t&&!0)q.Q=new A.w(Date.now(),!1)
J.aF(A.ae("settingsHelper.6",0),new B.cfl(q),y.e)
v=!0
x=1
break
case 1:return A.f(v,w)}})
return A.h($async$a3C,w)},
PX(d,e){return this.bJR(d,e)},
bJR(d,e){var x=0,w=A.i(y.h),v,u=this,t,s,r
var $async$PX=A.d(function(f,g){if(f===1)return A.e(g,w)
while(true)switch(x){case 0:s=A.c("saveChanges",!0)
r=A.B(A.c("theChangesWillBeDiscarded",!1),C.n,!1,!1,null,!1,!1,null,!1,!1,!1,1,null,!1,15,null,"")
r=A.K(A.a([r,e!=null?u.bwO(e):new A.m(null,null,null,null)],y.u),C.e,C.h,C.d)
t=A.c("save",!0)
x=3
return A.b(A.cQ(C.am,!1,1,!0,null,A.c("cancel",!0),null,r,d,C.N,20,!0,C.A,new B.cfj(d),new B.cfk(d),!1,t,s),$async$PX)
case 3:v=g
x=1
break
case 1:return A.f(v,w)}})
return A.h($async$PX,w)},
bwO(d){return new A.P(d,new B.cfi(d),null,null,y.p)}}
var z=a.updateTypes(["en(z,pA,k?)","~(pA)","~(pA?)","o?(o?)"])
B.cfp.prototype={
$1(d){var x=this.a.f
x.sj(0,d)
x.D()},
$S:36}
B.cff.prototype={
$0(){var x=0,w=A.i(y.e),v,u=this,t
var $async$$0=A.d(function(d,e){if(d===1)return A.e(e,w)
while(true)switch(x){case 0:t=u.a
x=3
return A.b(t.a3C(0,u.b,t.e),$async$$0)
case 3:v=e
x=1
break
case 1:return A.f(v,w)}})
return A.h($async$$0,w)},
$S:39}
B.cfe.prototype={
$0(){return A.dv(this.a)},
$S:0}
B.cfo.prototype={
$3(d,e,f){var x=null,w=this.b
if($.I().w.p3.at===w.a)return new A.m(32,x,x,x)
return new A.a4(new A.x(2,0,2,0),A.d2(A.br(G.f5,x,x,x,27),new B.cfn(this.a,d,w),0.5),x)},
$S:1316}
B.cfn.prototype={
$0(){var x=0,w=A.i(y.v),v,u=this,t,s,r,q,p
var $async$$0=A.d(function(d,e){if(d===1)return A.e(e,w)
while(true)switch(x){case 0:s=$.I()
r=s.w.p3
q=r.at!==""&&r.ax
p=new A.M(D.tg,$.ak(),y.i)
r=q?p:null
x=3
return A.b(u.a.PX(u.b,r),$async$$0)
case 3:if(e!==!0){x=1
break}t=A.kC(s.w.p3)
r=u.c
t.at=r.a
if(q&&!0)t.Q=new A.w(Date.now(),!1)
J.aF(A.ae("settingsHelper.6",0),new B.cfm(t),y.e)
r.D()
x=1
break
case 1:return A.f(v,w)}})
return A.h($async$$0,w)},
$S:3}
B.cfm.prototype={
$1(d){A.ac("settingsHelper.6")
return E.fc().pG(this.a)},
$S:4}
B.cfl.prototype={
$1(d){A.ac("settingsHelper.6")
return E.fc().pG(this.a)},
$S:4}
B.cfj.prototype={
$0(){A.O(this.a,!1).S(!1)
return null},
$S:0}
B.cfk.prototype={
$0(){A.O(this.a,!1).S(!0)
return null},
$S:0}
B.cfi.prototype={
$3(d,e,f){var x=null,w=y.u,v=A.a([],w)
C.b.p(D.aXT,new B.cfh(v,this.a))
return A.ai(x,x,x,A.K(A.a([A.fn(!1,x,!0,!1,!1,!0,!1,14,A.c("needToConfirmExplain",!1)),A.K(v,C.e,C.h,C.d)],w),C.e,C.h,C.d),x,x,0,!1,x,x,x,x,new A.x(0,10,0,0),!1,x,x,new A.x(5,8,5,8),x,x,!1,!1,x)},
$S:z+0}
B.cfh.prototype={
$1(d){var x,w=null,v=$.dlQ().h(0,d)
v.toString
v=A.B(A.c(v,!0),w,!1,!1,w,!1,!1,w,!1,!1,!1,1,w,!1,13,w,"")
x=this.b
this.a.push(F.ayP(new A.x(5,0,5,0),w,x.a,!1,new B.cfg(x),v,d,y.B))},
$S:z+1}
B.cfg.prototype={
$1(d){var x
if(d!=null){x=this.a
x.sj(0,d)
x.D()}},
$S:z+2};(function installTearOffs(){var x=a._static_1
x(B,"dML","dGF",3)})();(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.pA,A.fk)
x(B.WJ,A.a2)
x(B.aIi,A.aa)
w(A.b_,[B.cfp,B.cfo,B.cfm,B.cfl,B.cfi,B.cfh,B.cfg])
w(A.b2,[B.cff,B.cfe,B.cfn,B.cfj,B.cfk])})()
A.aS(b.typeUniverse,JSON.parse('{"WJ":{"a2":[],"k":[]},"aIi":{"aa":["WJ"]}}'))
var y={u:A.t("E<k>"),B:A.t("pA"),p:A.t("P<pA>"),C:A.t("P<o>"),i:A.t("M<pA>"),o:A.t("M<o>"),e:A.t("H"),h:A.t("H?"),v:A.t("~")};(function constants(){var x=a.makeConstList
D.tg=new B.pA(0,"change")
D.Nr=new B.pA(1,"notChange")
D.aXT=A.a(x([D.tg,D.Nr]),A.t("E<pA>"))})();(function lazyInitializers(){var x=a.lazy
x($,"dWP","dlQ",()=>A.F([D.tg,"LastDateToUpdateTermStateChange",D.Nr,"LastDateToUpdateTermStateNotChange"],y.B,A.t("o")))})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_416",e:"endPart",h:b})})($__dart_deferred_initializers__,"dtvbdoL2mKk3jSCKBX30YS9j76I=");