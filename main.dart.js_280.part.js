((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_280",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,D,B={x6:function x6(d,e){this.a=d
this.b=e},a25:function a25(d,e,f,g){var _=this
_.c=d
_.d=e
_.e=f
_.a=g},bx_:function bx_(d){this.a=d},bwZ:function bwZ(d){this.a=d}},C
A=c[0]
D=c[2]
B=a.updateHolder(c[244],B)
C=c[443]
B.x6.prototype={
P(){return"LoadingSwitchStatus."+this.b}}
B.a25.prototype={
n(d){return new A.W(this.c,new B.bx_(this),null,null,y.d)}}
var z=a.updateTypes(["Go(B,x6,l?)"])
B.bx_.prototype={
$3(d,e,f){var x,w,v=null
if(e===C.ye){x=this.a.e
return A.ym(new A.m(x,x,new A.hB("assets/animations/loading_animation.json.zip",x,x,D.bn,v),v),A.Y(0,0,0,500,0,0),D.Q,D.Q,A.FT())}x=A.Y(0,0,0,500,0,0)
w=$.lN()
return A.ym(A.am(v,A.fi(A.q(d).ax.f,new B.bwZ(this.a),e===C.hv),D.j,v,v,v,v,20,v,new A.y(w,0,w,0),v,v,v,25),x,D.Q,D.Q,A.FT())},
$S:z+0}
B.bwZ.prototype={
$1(d){return this.aL9(d)},
aL9(d){var x=0,w=A.j(y.f),v=this,u,t
var $async$$1=A.e(function(e,f){if(e===1)return A.f(f,w)
while(true)switch(x){case 0:u=v.a
t=u.c
t.sj(0,C.ye)
t.D()
x=2
return A.c(u.d.$1(d),$async$$1)
case 2:if(f)t.sj(0,d?C.hv:C.kG)
else t.sj(0,d?C.kG:C.hv)
t.D()
return A.h(null,w)}})
return A.i($async$$1,w)},
$S:46};(function inheritance(){var x=a.inherit,w=a.inheritMany
x(B.x6,A.fn)
x(B.a25,A.A)
w(A.b8,[B.bx_,B.bwZ])})()
A.b_(b.typeUniverse,JSON.parse('{"a25":{"A":[],"l":[]}}'))
var y={d:A.v("W<x6>"),f:A.v("~")};(function constants(){C.hv=new B.x6(0,"on")
C.kG=new B.x6(1,"off")
C.ye=new B.x6(2,"loading")})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_280",e:"endPart",h:b})})($__dart_deferred_initializers__,"ewcaudxrwL4nLdP+Lq5S3UZO6rc=");